...
class RepoSubmission {
  ...
  async handleRequest(issue) {
    ...
    if (daysCreated > 21) {
      this.commentAndClose(issue.number, 'Submission older than 21 days, you can re-submit your module if you want.');
      return;
    }
    try {
      const module = await new Module(url).load();
      await MagiskRepo.checkDuplicate(module.id);
      if (await module.isLegacy()) {
        this.commentAndClose(issue.number,
          'New module submissions are required to follow the new module installer format.'
          + 'Instructions on how to create new modules with the new format is available in this page: '
      ...
      if (issue.labels.length === 0) {
        // Only perform sanitize check if not labeled
        return;
      }
