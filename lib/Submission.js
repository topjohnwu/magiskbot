...
class RepoSubmission {
  ...
  async commentAndClose(issueId, comment) {
    ...
    try {
      await this.repo.createIssueComment(issueId, comment);
      await this.repo.editIssue(issueId, { state: 'closed' });
    } catch (err) {
      console.log(err);
    }
  }
  ...
  async handleRequest(issue) {
    ...
    try {
      ...
      const dup = await MagiskRepo.checkDuplicate(module.id);
      if (dup) { throw dup; }
      if (!await module.isNew()) { errno.throw(errno.ENOFILE, 'install.sh'); }
      console.log(`#${issue.number}: [SUBMISSION] [${module.name}] is valid`);
      if (issue.labels.length === 0) {
        // Perform only sanitize check if not labeled
        return;
      }
      if (issue.labels.includes('rejected')) {
        this.commentAndClose(issue.number, 'Submission rejected');
      } else if (issue.labels.includes('approved')) {
  ...
  async clearRequests() {
    try {
      (await this.issues.listIssues()).data.forEach(this.handleRequest);
    } catch (e) {
