...
// eslint-disable-next-line implicit-arrow-linebreak indent
    ...
    .catch((err) => {
      ...
      switch (errno.code(err)) {
        ...
        case errno.EOUTDATE: {
          ...
          const ri = gh.getIssues(ORGANIZATION, repo.name);
          ri.listIssues({ creator: MAGISKBOT })
            ...
            .then((issues) => {
              ...
              issues.forEach((issue) => {
                // Check time
                if (((Date.now() - Date.parse(issue.created_at)) / (1000 * 60 * 60 * 24)) > 14) {removeRepo = true;}
              });
              if (removeRepo) {
                gh.getRepo(ORGANIZATION, repo.name).deleteRepo().catch();
